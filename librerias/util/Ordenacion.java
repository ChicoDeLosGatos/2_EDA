...
public class Ordenacion {
    ...
    */
     ...
     * 
     * 2 casos base: 
     *  - caso 1 array con dos elementos [y+x, x]: Devolver array ordenado [x, y+x]
     *  - caso 2 array con un elemento [x]: Devolver array con un elemento
     *  
     *  Apunte: Queria inicializar T[] con los valores, pero me daba un error de
     *  generic array error, de manera que sabiendo que T extiende de comparable
     *  he creado la array de Comparable y la he casteado a T[].
     * 
     * @param v  Sus elementos implementan la interfaz Comparable
     * @param i  Extremo inferior del intervalo a ordenar
     * @param f  Extremo superior del intervalo a ordenar
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
        ...
        if(i + 1 == f)
            if(v[i].compareTo(v[f]) < 0)
                aux = (T[]) new Comparable[] {v[i], v[f]};
            else
                aux = (T[]) new Comparable[] {v[f], v[i]};
        else if (i==f) aux = (T[]) new Comparable[] {v[i]};
        else {
            int x = (i + f) / 2;
            T[] v1 = mergeSort2(v, i, x);
            T[] v2 = mergeSort2(v, x+1, f);
            aux = merge2(v1, v2);
        }
        return aux;
    }        
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
       int a = 0, b = 0, k = 0;
       T[] v = (T[]) new Comparable[v1.length + v2.length];
       
       while (a < v1.length && b < v2.length)
           v[k++] = (v1[a].compareTo(v2[b]) < 0) ? v1[a++] : v2[b++];
       while (a < v1.length) v[k++] = v1[a++];
       while (b < v2.length) v[k++] = v2[b++];
