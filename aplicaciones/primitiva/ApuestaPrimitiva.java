...
public class ApuestaPrimitiva {
    ...
    public ApuestaPrimitiva(boolean ordenada) {
        combinacion = (ordenada) ? new LEGListaConPIOrdenada() : new LEGListaConPI();
        for(int x = 0; x < NUMEROS; x++){
            NumeroPrimitiva np = new NumeroPrimitiva();
            while( posicionDe(np)>= 0){
    ...
    protected int posicionDe(NumeroPrimitiva n) {
        ...
        while(!combinacion.esFin()) {
            if(combinacion.recuperar().compareTo(n) == 0) return index;
            index++;
    ...
    public String toString() {
       String result = "";
       combinacion.inicio();
       for(int x = 0; x < NUMEROS-1; x++){
           result += combinacion.recuperar().toString() + ", ";
           combinacion.siguiente();
       }
       result += combinacion.recuperar().toString();
       return result;
