...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> terminosUnicos() {
        ListaConPI<Termino> terminos = new LEGListaConPI<Termino>();
        ListaConPI<Termino> claves = index.claves();
        claves.inicio();
        for(Termino k; !claves.esFin(); claves.siguiente()){
            k = claves.recuperar();
            if(index.recuperar(k).talla() == 1) 
                terminos.insertar(k);
        }
        return (terminos.talla() > 0) ? terminos : null;
    }
}    